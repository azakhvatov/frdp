...
//
// Original EN revision: SVN 44653
//
...
В зависимости от выбранного способа установки FreeBSD также может потребоваться поддерживаемый привод CDROM, а в некоторых случаях и сетевой адаптер.
...
[TIP]
====
Если копия FreeBSD на CDROM, DVD, или USB флеш-накопителе уже имеется, то этот раздел можно пропустить.
====

CD- и DVD-образы FreeBSD являются загрузочными. Для установки необходим один из них. Запишите образ на CD или DVD диск при помощи программы для записи CD, которая есть в используемой операционной системе. Во FreeBSD запись дисков осуществляется утилитой man:cdrecord[1] из комплекта [.filename]#sysutils/cdrtools# Коллекции Портов.
...
+
. FreeBSD начнёт загружаться. При загрузке с CDROM будут выдаваться сообщения, подобные следующим (информация о версиях опущена):
+
[source,shell]
....
Booting from CD-ROM...
...
Для установки FreeBSD, загружаемой по сети или с CDROM, потребуется доступ к PROM (OpenFirmware).
...
Установочный носитель может использоваться одним из трёх способов: для установки FreeBSD, как загрузочный Live CD, как описано в разделе <<using-live-cd>>, или просто для доступа к командной строке FreeBSD.
...
Преобразователь имён _DNS_ (_Domain Name System_) выполняет преобразование имён хостов в сетевые адреса и наоборот, преобразование сетевых адресов в имена хостов.
...
Ядро на установочных дисках сконфигурировано исходя из предположения, что большинство устройств находятся в настройках по умолчанию касательно прерываний, адресов ввода/вывода, каналов DMA.
