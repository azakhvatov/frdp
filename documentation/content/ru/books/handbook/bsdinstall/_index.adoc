...
//
// Original EN revision: SVN 44866
//
...
Начиная с FreeBSD 9.0-RELEASE, система поставляется с простой в использовании программой установки с текстовым интерфейсом, которая называется bsdinstall.
В этой главе описана установка FreeBSD при помощи bsdinstall.
Работа с sysinstall, являющейся программой установки, используемой во FreeBSD 8.x, описана в разделе <<install>>.
...
// * Разбиение и именование разделов жёстких дисков во FreeBSD.
...
Требования к оборудованию, необходимому для установки FreeBSD, различаются в зависимости от версии FreeBSD и аппаратной архитектуры.
Аппаратные архитектуры и устройства, поддерживаемые каким-либо релизом FreeBSD, перечислены в файле Примечаний об оборудовании.
Обычно именованный как [.filename]#HARDWARE.TXT#, этот файл размещается в корневом каталоге носителя с релизом.
Копии списка поддерживаемого оборудованиям также доступны на странице Информации о релизе на веб-сайте FreeBSD (link:https://www.FreeBSD.org/releases/index.html[https://www.FreeBSD.org/releases/index.html].

Для установки FreeBSD в самой минимальной конфигурации потребуется не менее 64 МБ ОЗУ и 1,1 ГБ свободного дискового пространства.
Для того, чтобы соответствовать потребностям приложений, которые будут использоваться, и объёмам данных, которые будут храниться, рекомендуется увеличивать объём ОЗУ и дискового пространства.
Требования к процессору для каждой архитектуры можно кратко изложить следующим образом:

AMD64::
Имеются два класса процессоров, на которых может работать версия для AMD64.
К первому классу относятся процессоры AMD64, в том числе AMD Athlon и AMD Opteron.

Во второй класс процессоров входят те, что используют архитектуру Intel(R) EM64T.
Примерами таких процессоров могут служить все многоядерные процессоры Intel(R) Xeon(TM), за исключением Sossaman, одноядерные процессоры Intel(R) Xeon(TM) Nocona, Irwindale, Potomac и Cranford, Intel(R) Core(TM) 2 (не Core Duo) и более поздние процессоры, все процессоры Intel(R) Pentium D, Intel(R) Pentium 4 и Celeron D, использующие ядро Cedar Mill, а также некоторые из Intel(R) Pentium 4 и Celeron D, использующие ядро Prescott.

Поддерживаются конфигурации как с однопроцессорной (UP), так и с симметричной многопроцессорной обработкой (SMP).
...
FreeBSD будет использовать преимущества технологии PAE (Physical Address Extensions) при работе на ЦПУ, поддерживающими эту функцию.
Ядро с включённой функцией PAE будет распознавать память выше 4 ГБ и обеспечит её использование системой.
Эта технология накладывает ограничения на драйверы устройств и другие возможности FreeBSD, которые могут быть задействованы; обратитесь за получением подробной информации к man:pae[4].

ia64::
Поддерживаемыми на данный момент процессорами являются Itanium и Itanium 2.
В списке поддерживаемых наборов микросхем входят HP zx1, Intel(R) 460GX и Intel(R) E8870.
Поддерживаются конфигурации как однопроцессорные (UP), так и с симметричной многопроцессорной обработкой (SMP).

pc98::
Поддерживаются все линейки NEC PC-9801/9821 с практически любыми процессорами, совместимыми с i386, в том числе 80486, Pentium, Pentium Pro и Pentium II.
Поддерживаются также все совместимые с i386 процессоры производства AMD, Cyrix, IBM и IDT.
Поддерживаются линейки EPSON PC-386/486/586, совместимые с NEC PC-9801.
Должны поддерживаться линейки NEC FC-9801/9821 и NEC SV-98.

Режим высокого разрешения не поддерживается.
Линейки NEC PC-98XA/XL/RL/XL^2 и NEC PC-H98 поддерживаются только в обычном (совместимом с PC-9801) режиме.
Функции SMP, имеющиеся во FreeBSD, не поддерживаются.
Шина NESA (New Extend Standard Architecture), используемая в линейках PC-H98, SV-H98 и FC-H98, не поддерживается.

powerpc::
Поддерживаются все системы Apple(R) Macintosh типа New World со встроенным USB.
Для многопроцессорных машин есть поддержка SMP.

32-разрядное ядро может использовать лишь первые 2 ГБ ОЗУ.

sparc64::
Системы, поддерживаемые FreeBSD/sparc64, перечислены в Проекте FreeBSD/sparc64 (link:https://www.FreeBSD.org/platforms/sparc/[https://www.FreeBSD.org/platforms/sparc.html].
...
Жёсткие диски могут быть разделены на несколько частей.
Эти части называются _разделами_.

Есть два способа деления диска на разделы.
Традиционный способ - _Master Boot Record_ (MBR) - хранит таблицу разделов, вмещающую до четырех _первичных разделов_.
Исторически сложилось так, что во FreeBSD эти разделы называют _слайсами_.
Возможны ситуации, в которых четыре раздела недостаточно, поэтому один из первичных разделов может быть превращен в _расширенный раздел_. Внутри расширенного раздела может быть создано несколько _логических разделов_. Результирующая структура выглядит немного неуклюже, но такова она есть.
...
[WARNING]
====
Некоторые старые операционные системы, например, Windows(R) XP, не совместимы со схемой разделов GPT.
Если на один диск необходимо установить FreeBSD совместно с такой операционной системой, то следует воспользоваться схемой MBR.
====

Стандартному загрузчику FreeBSD необходим первичный раздел (MBR) или GPT раздел.
Обратитесь к <<boot>> за более подробной информацией о процессе загрузки FreeBSD.
Если все первичные или GPT разделы уже задействованы, то для FreeBSD один из них необходимо будет освободить.
...
GParted Live (link:http://gparted.sourceforge.net/livecd.php[http://gparted.sourceforge.net/livecd.php] является свободно распространяемым загрузочным диском Live CD, в который включён редактор разделов GParted.
GParted включён также во многие другие дистрибутивы Linux формата Live CD.
...
Определенные трудности составляет изменение размеров разделов Microsoft(R) Vista. В таких случаях может пригодиться установочный CD от самой Microsoft(R) Vista.
...
По мере обнаружения и исправления этих проблем они описываются в Списке найденных ошибок FreeBSD (link:https://www.FreeBSD.org/releases/{rel120-current}R/errata/[https://www.FreeBSD.org/releases/{rel120-current}R/errata/]), размещаемом на веб-сайте FreeBSD.
...
Копии образов установочных носителей находятся по адресу link:https://www.FreeBSD.org/where.html#download[https://www.FreeBSD.org/where.html#download].
Также, в каталоге с файлами установочных образов находится файл [.filename]#CHECKSUM.SHA256#, который понадобится вам для проверки целостности скачанного файла образа. Проверка целостности файла образа производится сравнением _контрольных сумм_. Для подсчета последних FreeBSD предоставляет man:sha256[1], другие операционные системы также располагают подобными программами. Сравните полученную контрольную сумму с одной из [.filename]#CHECKSUM.SHA256#. Контрольные суммы должны совпасть полностью. Несовпадение контрольных сумм значит, что файл поврежден и к использованию не пригоден.

[TIP]
====
Если копия FreeBSD на CD, DVD, или флеш-накопителе USB уже имеется, то этот раздел можно пропустить.
====

CD- и DVD-образы FreeBSD являются загрузочными. Для установки необходим один из них. Запишите образ на CD или DVD диск при помощи программы для записи CD, которая есть в используемой операционной системе. Во FreeBSD запись дисков осуществляется утилитой `cdrecord` из пакета package:sysutils/cdrtools[], устанавливаемого из Коллекции Портов.
...
+ 
Образы для флеш-накопителя для FreeBSD 9.0-RELEASE и более поздних могут быть скачаны с каталога [.filename]#ISO-IMAGES/# по адресу
`ftp://ftp.FreeBSD.org/pub/FreeBSD/releases/_arch_/_arch_/ISO-IMAGES/_version_/FreeBSD-_version_-RELEASE-_arch_-memstick.img`.
Замените _arch_ и _version_ на, соответственно, архитектуру и номер версии, которую планируется установить.
Например, образы для флеш-накопителей FreeBSD/i386 9.0-RELEASE находятся на link:ftp://ftp.FreeBSD.org/pub/FreeBSD/releases/i386/i386/ISO-IMAGES/9.0/FreeBSD-9.0-RELEASE-i386-memstick.img[ftp://ftp.FreeBSD.org/pub/FreeBSD/releases/i386/i386/ISO-IMAGES/9.0/FreeBSD-9.0-RELEASE-i386-memstick.img].
+
[TIP]
======
...
В нижеследующем примере показано использование [.filename]#/dev/da0# в качестве устройства, на которое производится запись.
Удостоверьтесь в том, что целевое устройство выбрано верно, так как эта команда уничтожит существующие данные.
****
.. Запись образа при помощи man:dd[1]
+ 
Файл [.filename]#.img# не является обычным файлом. Это _образ_ всего содержимого флеш-накопителя. Этот файл _не может_ быть скопирован подобно обычному файлу, но должен быть записан непосредственно на целевое устройство при помощи команды man:dd[1]:
+
[source,shell]
....
# dd if=FreeBSD-9.0-RELEASE-i386-memstick.img of=/dev/_da0_ bs=64k
....
======
+
[.procedure]
...
[WARNING]
****
Удостоверьтесь в правильности выбора буквы диска, указываемого в качестве целевого устройства, так как имеющиеся данные будут перезаписаны и уничтожены.
****
...
Из программы установки можно выйти в любой момент до появления этого предупреждения, при этом содержимое дисков изменено не будет.
Если есть обеспокоенность относительно корректности настроек, просто выключите компьютер до этого сообщения, и ущерб не будет нанесён.
====
...
. При использовании "загрузочного" USB-накопителя, подготовленного ранее в соответствии с описанием в <<bsdinstall-installation-media>>, вставьте его в разъём USB перед включением компьютера.
+ 
При загрузке с CD включите компьютер и при первой же возможности вставьте CD.
. Настройте компьютер на загрузку с CD или с USB в зависимости от того, какой носитель используется для установки. Настройки BIOS позволяют выбрать конкретное загрузочное устройство. Большинство систем также предоставляют возможность выбрать загрузочное устройство во время запуска, часто эта возможность активируется по нажатию клавиши kbd:[F10], kbd:[F11], kbd:[F12] или kbd:[Escape].
. Если компьютер загружается как обычно и запускает существующую операционную систему, то:
.. Диск был вставлен слишком поздно. Оставьте его в приводе и попробуйте перезагрузить компьютер.
.. Изменения, внесённые ранее в параметры BIOS, не сработали. Повторяйте этот шаг, пока не будут подобраны правильные параметры.
.. Имеющийся BIOS не поддерживает загрузку с желаемого носителя. В таком случае для загрузки более старых машин с CD или USB можно использовать Plop Boot Manager (link:http://www.plop.at/en/bootmanager.html[http://www.plop.at/en/bootmanager.html]).
+
. FreeBSD начнёт загружаться. При загрузке с CD будут выдаваться сообщения, подобные следующим (информация о версиях опущена):
+
[source,shell]
....
Booting from CD-ROM...
...
На большинстве машин удерживание клавиши kbd:[C] на клавиатуре во время начальной загрузки активирует загрузку с CD.
Если это не так, то удерживайте kbd:[Command+Option+O+F] или kbd:[Windows+Alt+O+F] на не-Apple(R) клавиатурах. На приглашение `0 >` введите
...
Большинство систем sparc64 настроены на автоматическую загрузку с жёсткого диска.
Для установки FreeBSD, загружаемой по сети или с CD, потребуется доступ к PROM (OpenFirmware).
...
Если в этот момент компьютер продолжает загружаться с жёсткого диска, то для получения приглашения PROM вам необходимо нажать на клавиатуре kbd:[L1+A] или kbd:[Stop+A], или же послать сигнал `BREAK` через последовательную консоль (используя, например, `~#` в man:tip[1] или man:cu[1]). Приглашение выглядит подобно следующему:
...
<.> Это приглашение, используемое в системах с одним ЦПУ.

<.> Это приглашение, отображающееся на многопроцессорных (SMP) системах, а цифра указывает на количество активных ЦПУ.
...
Чтобы просмотреть содержимое буфера, нажмите kbd:[Scroll Lock]. Это включит режим буфера прокрутки.
Для просмотра сообщений используйте клавиши навигации или клавиши kbd:[PageUp] и kbd:[PageDown]. Чтобы выйти из режима просмотра буфера, нажмите ещё раз kbd:[Scroll Lock].

Включите прокрутку экранного буфера и просмотрите сообщения, которые были вытеснены с экрана во время определения устройств ядром.
Будет отображаться текст, подобный <<bsdinstall-dev-probe>>, однако его содержимое будет отличаться и зависеть от набора установленных в компьютер устройств.
...
Внимательно изучите вывод определения устройств и убедитесь, что FreeBSD обнаружила все предполагаемые устройства.
Если устройство не было найдено, то оно не будет упомянуто в выдаче.
Обратитесь к разделу <<kernelconfig-custom-kernel>> за инструкциями по добавлению поддержки устройств, отсутствующих в ядре [.filename]#GENERIC#.

После процедуры определения устройств будет отображено меню, показанное в <<bsdinstall-choose-mode>>.
Установочный носитель может использоваться одним из трёх способов: для установки FreeBSD, в качестве загрузочного Live CD, как описано в разделе <<using-live-cd>>, или для доступа к командной строке FreeBSD.
Используйте клавиши навигации для выбора опции и kbd:[Enter] для его подтверждения.
...
[NOTE]
====
В комплекте с PC-BSD поставляется программа pc-sysinstall от {kmoore}, которая может использоваться для установки FreeBSD в соответствии с инструкциями по адресу link:http://wiki.pcbsd.org/index.php/Use_PC-BSD_Installer_to_Install_FreeBSD[http://wiki.pcbsd.org/index.php/Use_PC-BSD_Installer_to_Install_FreeBSD].
Несмотря на то, что эту программу иногда путают с bsdinstall, они между собой не связаны.
====
...
Во FreeBSD 10.0-RELEASE и более поздних версиях это меню было улучшено.
Показывается полный список клавиатурных раскладок с уже выбранной применяемой по умолчанию.
Кроме того, при выборе другой раскладки отображается диалоговое окно, позволяющее пользователю проверить эту раскладку и удостовериться в её корректной работе до того, как продолжить.

<<bsdinstall-keymap-10>>

[[bsdinstall-keymap-10]]
.Keymap Selection Menu
image::bsdinstall-keymap-10.png[Улучшенное меню раскладки клавиатуры]
...
Ядро и набор пользовательских утилит FreeBSD, вместе называемые "базовой системой", устанавливаются всегда.
...
Есть четыре способа осуществить распределение дискового пространства для FreeBSD.
_Шаблонное_ (_guided_) разбиение автоматически настраивает разделы диска, _ручное_ (_manual_) разбиение позволяет опытным пользователям создавать разделы в соответствии с собственными потребностями.
Вариант ZFS, доступный только во FreeBSD 10 и более поздних версиях, приведёт к созданию опционально зашифрованной корневой системы в формате ZFS с поддержкой механизма Boot Environments.
Наконец, есть возможность вызвать интерпретатор командной строки, в котором можно будет непосредственно запускать такие утилиты, как man:gpart[8], man:fdisk[8] и man:bsdlabel[8].

[[bsdinstall-part-guided-manual]]
.Выбор способа разбиения: шаблонное (guided), ручное (manual) или из командной строки
image::bsdinstall-part-guided-manual.png[]

[[bsdinstall-zfs-partmenu]]
.Выбор способа разбиения: шаблонное (guided), ручное (manual), из командной строки или ZFS
...
Затем показывается, как пользоваться экранными формами в случае как Ручного Разбиения, так и Управляемого Разбиения.
...
При установке больших пакетов програмного обеспечения, подобных Firefox, OpenOffice или LibreOffice, могут возникнуть сложности, если в каталоге [.filename]#/var/tmp# будет недостаточно дискового пространства.
...
Для этого раздела рекомендуется выделять по меньшей мере 2 гигабайта.
...
Для FreeBSD может быть выделен как весь диск, так и его часть.
Если выбирается btn:[Entire Disk], то создаётся стандартное разбиение, занимающее весь диск. Выбрав btn:[Partition], вы получите создание разделов в неиспользуемой области диска.
...
|APM
|Карта разделов Apple, используемая в PowerPC(R) Macintosh(R) (link:http://support.apple.com/kb/TA21692[http://support.apple.com/kb/TA21692].
...
|PC98
|Разновидность MBR, используемая в компьютерах NEC PC-98 (link:http://en.wikipedia.org/wiki/Pc9801[(link:http://en.wikipedia.org/wiki/Pc9801].
...
Ещё одним типом раздела, который следует упомянуть, является `freebsd-zfs`, используемый для разделов, которые будут содержать файловую систему FreeBSD ZFS.
<<filesystems-zfs>>.
Сведения об имеющихся в наличии типах разделов GPT содержатся в man:gpart[8].

Возможно создание большего количества разделов с файловыми системами, и некоторые пользователи предпочитают выделять отдельные разделы для таких файловых систем, как [.filename]#/#, [.filename]#/var#, [.filename]#/tmp#, и [.filename]#/usr#. Иллюстрация подобного разбиения приведена в <<bsdinstall-part-manual-splitfs>>.
...
Программа установки автоматически создаст разделы, границы которых кратны 4Кб, а также принудительно укажет ZFS на использование секторов размером 4Кб.
Это безопасно даже при использовании дисков с размером сектора в 512 байт, а также даёт определённый плюс, гарантируя то, что пулы, созданные на дисках с размером сектора в 512 байт, в будущем смогут работать с дисками с размером сектора в 4Кб, добавляемыми для расширения пространства хранения или в качестве замены вышедших из строя дисков.
...
Если включено шифрование, то создаётся незашифрованный загрузочный пул размером в 2ГБ, содержащий каталог [.filename]#/boot#, в котором хранятся ядро и другие файлы, необходимые для загрузки системы.
...
Основное меню ZFS в программе установки предлагает выбор из нескольких параметров, влияющих на создание пула.

[[bsdinstall-zfs-menu]]
.Меню формирования разделов ZFS
image::bsdinstall/bsdinstall-zfs-menu[]

Самым важным пунктом является выбор типа vdev и одного или большего количества дисков, из которых будет сформирован vdev, представляющий дисковый пул.
...
Подсказка внизу экрана при выборе типа дискового пула помогает понять, сколько дисков требуется для каждого из них, а в случае RAID-Z оптимальное количество дисков для каждой конфигурации.

[[bsdinstall-zfs-vdev_type]]
.Тип пула ZFS
image::bsdinstall/bsdinstall-zfs-vdev_type[]

Сразу после выбора типа дискового пула отображается список доступных дисков, а у пользователя запрашивается выбор одного или большего количества дисков для формирования этого пула.
...
Чтобы удостовериться в том, что был выбран корректный диск и данные не будут случайно уничтожены из-за ошибки выбора диска, может быть использовано меню btn:[- Disk Info] для проверки данных каждого диска, в том числе таблицы разделов и различной дополнительной информации, включая номер модели устройства и его серийный номер, если он доступен.

[[bsdinstall-zfs-disk_info]]
.Анализ диска
image::bsdinstall/bsdinstall-zfs-disk_info[]

Это меню также позволяет пользователю ввести название дискового пула, отключить принудительное использование секторов 4k, включить или отключить шифрование, переключаться между типами таблиц разделов GPT (рекомендуется) или MBR, а также выбрать желаемый размер раздела подкачки.
...
Если было включено шифрование диска man:geli[8], то программа установки дважды запросит парольную фразу, которая будет использоваться для шифрования дисков.
...
При создании сложных конфигураций может оказаться, что базовый интерфейс программы установки не обеспечивает требуемого уровня гибкости.
Выбор варианта btn:[Shell] в меню формирования дисковых разделов переведёт пользователя в режим командной строки.
Ожидается, что пользователь сформирует дисковые разделы на дисковых накопителях и создаст желаемые файловые системы, заполнит файл man:fstab[5] в [.filename]#/tmp/bsdinstall_etc/fstab# и смонтирует файловые системы в [.filename]#/mnt#.
После того, как эти действия будут завершены, выполнение команды `exit` приведёт к возврату в программу установки, а процесс установки продолжится с этапа распаковки установочных пакетов.
...
Для продолжения выберите btn:[Commit].
Если необходимо внести изменения, то для возвращения к редактору разделов выберите btn:[Back].
Выбор btn:[Revert & Exit] приведёт к выходу из программы установки без записи изменений на жёсткий диск.
...
Как только запрошенные файлы дистрибутива распакуются, bsdinstall приступит к выполнению послеустановочных конфигурационных задач, описанных в разделе <<bsdinstall-post>>.
...
Существуют два способа настройки протокола IPv4 на сетевом интерфейсе. Сервис _DHCP_ автоматически установит корректную конфигурацию сетевого интерфейса, и это - предпочтительный способ настройки. _Статическая_ конфигурация требует ручного ввода настроек протокола IPv4.
...
SLAAC позволяет компоненту сети запрашивать информацию, необходимую для автоматической настройки, у локального маршрутизатора.
За подробностями обратитесь к link:http://tools.ietf.org/html/rfc4862[http://tools.ietf.org/html/rfc4862].
...
Преобразователь имён DNS (Domain Name System) выполняет преобразование имён хостов в сетевые адреса и наоборот, преобразование сетевых адресов в имена хостов.
Если для автоматической настройки сетевого интерфейса использовались механизмы DHCP или SLAAC, то параметры преобразователя имён может уже присутствовать в разделе `Resolver Configuration`.
Если это не так, то введите название домена локальной сети в поле `Search`.
`DNS #1` и `DNS #2` являются IP-адресами локальных серверов DNS.
Необходимо указать по крайней мере один сервер.
...
Установка часового пояса для системы позволит ей автоматически корректировать время согласно местным законам и правильно выполнять остальные зависимые от часового пояса функции.

Демонстрируемый пример приведён для машины, находящейся в восточном часовом поясе Соединённых Штатов.
Варианты выбора будут отличаться в соответствии с географическим местонахождением.

[[bsdinstall-local-utc]]
.Выбор местного времени или времени UTC
image::bsdinstall-set-clock-local-utc.png[]

Выберите btn:[Yes] или btn:[No] согласно тому, как настроены часы вашего компьютера, далее нажмите kbd:[Enter].'
Если не известно, использует система UTC или местное время, то нажмите btn:[No] для того, чтобы выбрать наиболее часто встречающийся вариант с применением местного времени.
...
* `Invite _user_ into other groups?` - Дополнительные группы, в которые будет включён пользователь.
...
Корректное завершение работы компьютера с FreeBSD помогает защитить от повреждений не только данные, но даже и аппаратное обеспечение.
Не выключайте электропитание до полного завершения работы системы.
Если вы входите в группу `wheel`, то станьте суперпользователем набрав в командной строке команду `su` и введя пароль пользователя `root`. Или же, войдите в систему как `root` и наберите команду `shutdown -p now`. Система корректно завершит работу и выключится.
...
Ввиду различных ограничений архитектуры ПК, определение устройств не может быть достоверным на все 100%, однако есть несколько действий, которые можно предпринять, если обнаружение устройств завершится неудачно.

Просмотрите Информацию об оборудовании (link:https://www.FreeBSD.org/releases/index.html[https://www.FreeBSD.org/releases/index.html] для соответствующей версии FreeBSD, чтобы убедиться, что ваше оборудование поддерживается.
...
Ядро на установочных дисках сконфигурировано в предположении, что настройки большинства устройств соответствуют заводским параметрам, используемым по умолчанию для задания прерываний, адресов ввода/вывода и каналов DMA.
...
==== Моя система зависает во время загрузки на этапе обнаружения устройств, либо она ведёт себя странно во время установки.
...
FreeBSD Live CD находится на том же CD, что и установочная программа. Это удобно для тех пользователей, которые всё еще размышляют о пригодности для них ОС FreeBSD и желают проверить некоторые функциональные возможности до начала установки.
