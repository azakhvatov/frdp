...
//
// Original EN revision (13.02.2023): 4d1de1c7246d147e1c428b542350695b8fedf84e
//
...
weight: 7
path: "/books/handbook/"
aliases: ["/ru/books/handbook/x-understanding/","/ru/books/handbook/x-install/","/ru/books/handbook/x-config/","/ru/books/handbook/x-fonts/","/ru/books/handbook/x-xdm/","/ru/books/handbook/x11-wm/","/ru/books/handbook/x-compiz-fusion/","/ru/books/handbook/x11-troubleshooting/"]
---
...
Для добавления текущего пользователя в группу `video` выполните следующую команду:
...
Драйвер Intel(R) может быть установлен выполнением следующей команды:
...
Затем в файл `/etc/rc.conf` добавьте модуль, выполнив следующую команду: 
...
Для графических адаптеров, выпущенных позже HD7000 или Tahiti, добавьте модуль в файл `/etc/rc.conf` при помощи следующей команды:
...
Для более старых графических адаптеров (до HD7000 или до Tahiti) добавьте модуль в файл `/etc/rc.conf` при помощи запуска следующей команды:
...
Пользователям более новых графических адаптеров необходимо будет установить пакет package:x11/nvidia-driver[].
В случае более старого оборудования необходимо будет уточнять, какие версиисоедржат его поддержку.

.Поддерживаемыеы версии драйверов NVIDIA(R)
...
[WARNING]
====
Версия 304 драйверов графики NVIDIA(R) (package:x11/nvidia-driver-304[]) не поддерживает xorg-server версии 1.20 и выше.
====
...
[WARNING]
====
Драйвер `nvidia-modeset` должен использоваться *исключительно* в качестве альтернативы для `nvidia`, если запуск сервера X приводит к зависанию или в `/var/log/Xorg.0.log` появляются следующие строки:
...
При помощи пакета package:x11/libinput[] в Xorg обеспечивается поддержка подавляющего большинства устройств ввода.

[TIP]
====
Некоторые графические оболочки (такие как KDE Plasma) предоставляют графический пользовательский интерфейс для настройки этих паарметров.
